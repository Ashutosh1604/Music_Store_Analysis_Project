...
select c.customer_id,c.first_name, c.last_name, sum (total) as highest_spent 
from customer as c inner join invoice as i on c.customer_id=i.customer_id 
group by c.customer_id order by highest_spent desc limit 1
...
join genre gon g.genre_id=t.genre_id where g.name='Rock'
order by c.email
